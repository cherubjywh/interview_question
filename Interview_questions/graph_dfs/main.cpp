...
public:
    vector<int> findOrder(int numCourses, vector<pair<int, int>>& prerequisites) {
        ...
        while (!not_visited.empty()) {
            int current_node = *not_visited.begin();
            not_visited.erase(current_node);
            
            DFS(graph, current_node, F, not_visited, order);
        }
        
        
        for (auto & e : prerequisites) {
            if (order[e.second] > order[e.first]) {
                order.clear();
                return order;
            }
        }
...
private:
    void DFS(vector<unordered_set<int>>& graph, int current_node, int & F, unordered_set<int>& not_visited, vector<int>& order) {
        for (auto i : graph[current_node]) {
            if (not_visited.find(i) != not_visited.end()) {
                not_visited.erase(i);
                DFS(graph, i, F, not_visited, order);
            }
        }
...
int main(int argc, const char * argv[]) {
    ...
    Solution sl;
    auto res = sl.findOrder(num, edges);
